...
/**
 * The workout generator, moved off the home screen so home can stay a launcher.
 * All generator state lives in useWorkoutStore, so selections survive
...
export default function GenerateWorkoutScreen() {
  ...
  const updateDurationWithDefaults = useCallback(
    (duration: number) => {
  ...
  const selectedSet =
    equipmentSets.find((s) => s.id === selectedEquipmentSetId) ||
  ...
  const handleGenerateWorkout = async () => {
    ...
    try {
      ...
      // The user may have backed out during the ~15s generation. Unlike the old
      // always-mounted home tab, this is a pushed screen — yanking them onto
      // the review screen from wherever they are now would be a hijack. The
      // generated workout is kept in the store either way.
        ...
        <Card style={styles.section}>
          ...
          <ScrollView horizontal showsHorizontalScrollIndicator={false}>
            <View style={styles.chipRow}>
              ...
              <Chip label="Add Set" icon="add" onPress={() => router.push('/(tabs)/profile')} />
            </View>
          </ScrollView>
          ...
          <View style={styles.durationGrid}>
            {DURATION_OPTIONS.map((option) => {
              ...
              return (
                ...
                >
                  <Text
                    style={[styles.durationValue, isSelected && styles.durationValueSelected]}
                  >
                    {option.label}
                  </Text>
                </Pressable>
